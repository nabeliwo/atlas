...
| 2026-08-30 | 地図側の道路番号を非表示にし、道路名と小さい地名を減光する | 主役は「自分が行った場所」で地図は背景。素の positron は拡大するとピンより道路番号や地名の方が強く、序列が逆転していた。レイヤーIDは provider 固有なので `src/lib/map-config.ts` に隔離した |
| 2026-08-30 | ピンの当たり判定を見た目と切り離す | レイヤーに直接 click を張ると判定が円の内側だけになり、スマホで押しにくい。タップ位置のまわりに矩形を取って探す方式にした。タッチでは 16px、マウスでは 6px の余裕を取る。Place 名ラベルも同じ feature として拾う |
